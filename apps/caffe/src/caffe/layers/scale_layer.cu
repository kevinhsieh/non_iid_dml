...
#if (PARAM_DBG)
    LOG(INFO) << "Keeping bottom data for scale layer";
    int print_size = std::min(DBG_SIZE, temp_.count());
    Dtype print_buf[DBG_SIZE]; 
    std::stringstream value_out;
    caffe_copy(print_size, temp_.gpu_data(), print_buf);
    value_out << "Data Values: ";
...
#if (PARAM_DBG)
    LOG(INFO) << "Info for scale layer";
    LOG(INFO) << is_eltwise << "," << scale_param << "," << propagate_down[0]
              << "," << inner_dim_ << "," << outer_dim_;
    int print_size = std::min(DBG_SIZE, top[0]->count());
    Dtype print_buf[DBG_SIZE]; 
    std::stringstream value_out;
    caffe_copy(print_size, bottom_data, print_buf);
    value_out << "Bottom Data Values Before mul: ";
    for (int i = 0; i < print_size; i++) {
      value_out << print_buf[i] << ", ";
    }
    LOG(INFO) << value_out.str();
...
#if (PARAM_DBG)
    std::stringstream value_out2;
    caffe_copy(print_size, product, print_buf);
    value_out2 << "Product Data Values After mul: ";
    for (int i = 0; i < print_size; i++) {
      value_out2 << print_buf[i] << ", ";
    }
    LOG(INFO) << value_out2.str();
#endif
...
#if (PARAM_DBG)
        std::stringstream value_out3;
        print_size = std::min(DBG_SIZE, sum_result_.count());
        caffe_copy(print_size, sum_result, print_buf);
        value_out3 << "Sum Result value: ";
        for (int i = 0; i < print_size; i++) {
          value_out3 << print_buf[i] << ", ";
        }
        LOG(INFO) << value_out3.str();
#endif
...
#if (PARAM_DBG)
          std::stringstream value_out3;
          print_size = std::min(DBG_SIZE, scale->count());
          caffe_copy(print_size, scale_diff, print_buf);
          value_out3 << "Scale diff value: ";
          for (int i = 0; i < print_size; i++) {
            value_out3 << print_buf[i] << ", ";
          }
          LOG(INFO) << value_out3.str();
