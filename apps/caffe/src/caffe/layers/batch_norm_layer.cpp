...
void BatchNormLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  eps_ = param.eps();
  /* XXX: I don't want to keep the running average of mean and variance */
  ...
  for (int i = 0; i < this->blobs_.size(); ++i) {
    if (this->layer_param_.param_size() == i) {
      ...
      //CHECK_EQ(this->layer_param_.param(i).lr_mult(), FIX_LEARNING_RATE)
...
void BatchNormLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (!use_global_stats_) {
    ...
    caffe_copy(variance_.count(), variance_.cpu_data(), 
               variance_back_.mutable_cpu_data());
...
void BatchNormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  // compute and save moving average
  if (!use_global_stats_) {
    const Dtype* m_counter = this->blobs_[2]->cpu_data();    
    Dtype* m_counter_diff = this->blobs_[2]->mutable_cpu_diff();    

    const Dtype* m_average = this->blobs_[0]->cpu_data();
    Dtype* m_average_diff = this->blobs_[0]->mutable_cpu_diff();
    
    const Dtype* m_variance = this->blobs_[1]->cpu_data();
    Dtype* m_variance_diff = this->blobs_[1]->mutable_cpu_diff();
    int m = bottom[0]->count()/channels_;
    Dtype bias_correction_factor = m > 1 ? Dtype(m)/(m-1) : 1;

    // We only apply moving average fraction on worker 0
    if (0 == Caffe::worker_id()) {
      //*m_counter_diff += (moving_average_fraction_ - Dtype(1)) * (*m_counter);
      caffe_axpy(mean_.count(), moving_average_fraction_ - Dtype(1), 
                     m_average, m_average_diff);
      caffe_axpy(variance_back_.count(), moving_average_fraction_ - Dtype(1), 
                     m_variance, m_variance_diff);
    }

    *m_counter_diff += 1;
    caffe_axpy(mean_.count(), Dtype(1), mean_.cpu_data(), m_average_diff);
    caffe_axpy(variance_back_.count(), bias_correction_factor, 
                   variance_back_.cpu_data(), m_variance_diff);
  }
