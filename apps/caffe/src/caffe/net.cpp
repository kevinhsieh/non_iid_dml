  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    for (int top_id = 0; top_id < num_top; ++top_id) {
      if ((layer_param.type() == "LRN" && top_id > 0)
          ...
          || (layer_param.type() == "BatchReNorm" && top_id > 0)
    ...
    LOG(INFO) << "param size: " << param_size << ", param blobs: " << num_param_blobs;
    LOG(INFO) << "top size: " << top_vecs_[layer_id].size() 
              << ", bottom size: " << bottom_vecs_[layer_id].size();
...
void Net<Dtype>::AppendParam(const NetParameter& param, const int layer_id,
  ...
  LOG(INFO) << "Adding param " << net_param_id << " from layer " << layer_id
