...
def Inception7A(data,
    ...
    pooling = mxc.Pooling(data=data, kernel=(3, 3), stride=(1, 1), pad=(1, 1), pool_type=pool, name=('%s_pool_%s_pool' % (pool, name)))
    cproj = Conv(pooling, proj, name=('%s_tower_2' %  name), suffix='_conv')
    concat = mxc.Concat(bottoms=[tower_1x1, tower_5x5, tower_3x3, cproj], name='ch_concat_%s_chconcat' % name)
    return concat
...
def Inception7B(data,
    ...
    pooling = mxc.Pooling(data=data, kernel=(3, 3), stride=(2, 2), pad=(0,0), pool_type="max", name=('max_pool_%s_pool' % name))
    concat = mxc.Concat(bottoms=[tower_3x3, tower_d3x3, pooling], name='ch_concat_%s_chconcat' % name)
    return concat
...
def Inception7C(data,
    ...
    # concat
    concat = mxc.Concat(bottoms=[tower_1x1, tower_d7, tower_q7, cproj], name='ch_concat_%s_chconcat' % name)
    return concat
...
def Inception7D(data,
    ...
    pooling = mxc.Pooling(data=data, kernel=(3, 3), stride=(2, 2), pool_type=pool, name=('%s_pool_%s_pool' % (pool, name)))
    # concat
    concat = mxc.Concat(bottoms=[tower_3x3, tower_d7_3x3, pooling], name='ch_concat_%s_chconcat' % name)
    return concat
...
def Inception7E(data,
    ...
    cproj = Conv(data=pooling, num_filter=proj, kernel=(1, 1), name=('%s_tower_2' %  name), suffix='_conv')
    # concat
    concat = mxc.Concat(bottoms=[tower_1x1, tower_d3_a, tower_d3_b, tower_3x3_d3_a, tower_3x3_d3_b, cproj], name='ch_concat_%s_chconcat' % name)
    return concat
...
pool = mxc.Pooling(data=conv_2, kernel=(3, 3), stride=(2, 2), pool_type="max", name="pool")
# stage 2
conv_3 = Conv(pool, 80, kernel=(1, 1), name="conv_3")
conv_4 = Conv(conv_3, 192, kernel=(3, 3), name="conv_4")
pool1 = mxc.Pooling(data=conv_4, kernel=(3, 3), stride=(2, 2), pool_type="max", name="pool1")
# stage 3
...
in5b = Inception7E(in5a, 320,
                   ...
                   "max", 192, "mixed_10")
# pool
pool = mxc.Pooling(data=in5b, kernel=(8, 8), stride=(1, 1), pool_type="avg", name="global_pool")
flatten = mxc.Flatten(data=pool, name="flatten")
fc1 = mxc.FullyConnected(data=flatten, num_hidden=num_classes, name='fc1')
# softmax = mxc.SoftmaxOutput(data=fc1, name='softmax')
